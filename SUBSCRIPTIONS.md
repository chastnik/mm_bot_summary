...
```
каналы ~ время ~ частота
```

### Параметры:

#### **Каналы**
- Перечислите через запятую **без пробелов**
- Можно указывать с символом `~` или без него
- Примеры: `general,random` или `~general,~random`

#### **Время**
- Формат: `HH:MM` (24-часовой формат)
- Примеры: `09:00`, `18:30`, `12:15`
...
general,development ~ 09:00 ~ daily

# Еженедельная сводка в 18:00
marketing,sales ~ 18:00 ~ weekly

# Несколько каналов
general,random,development,qa ~ 12:00 ~ daily
```
...
1. **Бот должен быть добавлен** во все каналы из подписки
2. **Команда для добавления:** `/invite @bot_username`
3. **Проверка доступа:** бот автоматически проверяет права доступа
